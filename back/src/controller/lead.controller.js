...
const createLead = async (req, res) => {
    try {
        ...
        if (!companyName || !customerName || !amount || !productName || !emailAddress || !address || !date) {
            return res.status(400).json({
                success: false,
                message: "Required fields (companyName, customerName, amount, productName, emailAddress, address, date) are missing",
            });
        }

        // Additional email validation (if it wasn't covered in the schema)
